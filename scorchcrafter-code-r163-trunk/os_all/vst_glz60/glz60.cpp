...
{
	return new ScGLZ60 (audioMaster);
}
//! -------------------------------------------------------------------------------------------------------
ScGLZ60::ScGLZ60 (audioMasterCallback audioMaster) : AudioEffectX (audioMaster, 1, kMaxNumParameters),
	...
	outputDoubleR(double (0.0))
{
	...
	params[kPreEQstackLow]=float (0.50f);
	params[kPreEQstackMid]=float (0.50f);
	params[kPreEQstackHigh]=float (0.50f);
	params[kPreEQstackContour]=float (0.50f);
	params[kPreEQstackPres]=float (0.50f);
	...
	GlzAmp = new SC_GLZ60_Amp_Head(fsx (sRate));
...
#else
	setEditor (new PlgEditor_Glz (this));
#endif
#endif

	GlzAmp->SetIntParam(ScPrmI_OverSampling_Rate, SCv_OverSampling_2x);
	GlzAmp->SetIntParam(ScPrmI_FP_Precision_Bit_Depth, SCv_Bit_Depth_64);
	GlzAmp->SetFpParam(ScPrmF_Drive_Gain, params[kGainControl]);
	GlzAmp->SetFpParam(ScPrmF_Master_Volume, params[kMasterVol]);
	GlzAmp->SetFpParam(ScPrmF_EQ_Contour, params[kEQstackContour]);
	GlzAmp->SetFpParam(ScPrmF_EQ_Low, params[kEQstackLow]);
	GlzAmp->SetFpParam(ScPrmF_EQ_Mid, params[kEQstackMid]);
	GlzAmp->SetFpParam(ScPrmF_EQ_High, params[kEQstackHigh]);
	GlzAmp->SetFpParam(ScPrmF_EQ_Presence, params[kEQstackPres]);
	GlzAmp->SetFpParam(ScPrmF_Input_Vol, params[kInputVol]);
	GlzAmp->SetFpParam(ScPrmF_PreEQ_Contour, params[kPreEQstackContour]);
	GlzAmp->SetFpParam(ScPrmF_PreEQ_Low, params[kPreEQstackLow]);
	GlzAmp->SetFpParam(ScPrmF_PreEQ_Mid, params[kPreEQstackMid]);
	GlzAmp->SetFpParam(ScPrmF_PreEQ_High, params[kPreEQstackHigh]);
	GlzAmp->SetFpParam(ScPrmF_PreEQ_Presence, params[kPreEQstackPres]);
	GlzAmp->SetIntParam(ScPrmI_Drive_Channel, ampChan);
	GlzAmp->SetIntParam(ScPrmI_Stereo, 0);
	GlzAmp->SetIntParam(ScPrmI_CabMic_Active, 0);
	GlzAmp->SetIntParam(ScPrmI_CabMic_Num, 0);
	GlzAmp->SetIntParam(ScPrmI_TStack_Shape, 1);
	GlzAmp->SetIntParam(ScPrmI_TStack_ShapeNum, 0);
}
//! -------------------------------------------------------------------------------------------------------
ScGLZ60::~ScGLZ60()
{
...
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::processReplacing (float** inputs, float** outputs, VstInt32 sampleFrames)
{
	float* in1  = inputs[0];
	float* in2  = inputs[1];
...
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::processDoubleReplacing (double** inputs, double** outputs, VstInt32 sampleFrames)
{
	double* in1 = inputs[0];
	double* in2 = inputs[1];
...
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::setProgramName (char* name)
{
	vst_strncpy (programName, name, kVstMaxProgNameLen);
}
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::getProgramName (char* name)
{
	vst_strncpy (name, programName, kVstMaxProgNameLen);
}
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::setParameter (VstInt32 index, float value)
{
...
#else
		if(index < kMaxNumParameters) ((PlgEditor_Glz*)editor)->setParameter (index, val);
#endif
	}
#endif
}
//! -------------------------------------------------------------------------------------------------------
float ScGLZ60::getParameter (VstInt32 index)
{
...
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::getParameterLabel (VstInt32 index, char* label)
{
	if(index < kMaxNumParameters) vst_strncpy (label, " ", kVstMaxParamStrLen);
}
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::getParameterDisplay (VstInt32 index, char* text)
{
	if(index < kMaxNumParameters)
	{
...
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::getParameterName (VstInt32 index, char* text)
{
	vst_strncpy (text, " ", kVstMaxParamStrLen);
}
//! -------------------------------------------------------------------------------------------------------
bool ScGLZ60::getEffectName (char* name)
{
#ifdef COMPILER_MGW32
	vst_strncpy (name, "SC-x86 GLZ-60 Bass Amp", kVstMaxEffectNameLen);
#endif
#ifdef COMPILER_MGW64
	vst_strncpy (name, "SC-x64 GLZ-60 Bass Amp", kVstMaxEffectNameLen);
#endif
	return true;
}
//! -------------------------------------------------------------------------------------------------------
bool ScGLZ60::getProductString (char* text)
{
#ifdef COMPILER_MGW32
	vst_strncpy (text, "GLZ60-mg32H", kVstMaxProductStrLen);
#endif
#ifdef COMPILER_MGW64
	vst_strncpy (text, "GLZ60-mg64H", kVstMaxProductStrLen);
#endif
	return true;
}
//! -------------------------------------------------------------------------------------------------------
bool ScGLZ60::getVendorString (char* text)
{
	vst_strncpy (text, "ScorchCrafter", kVstMaxVendorStrLen);
	return true;
}
//! -------------------------------------------------------------------------------------------------------
VstInt32 ScGLZ60::getVendorVersion ()
{
	return 3200;
}
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::setSampleRate (float smplRate)
{
...
//	if(float (sRate) > float (768000.0f)) sRate=float (768000.0f);
	...
	GlzAmp->SetNewSampleRate(fsx (sRate));
}
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::open()
{
...
//! -------------------------------------------------------------------------------------------------------
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::ZeroSamples()
{
	GlzAmp->ClearAllBuffers();
};
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::RunFX(const fsx inpSplA, const fsx inpSplB)
{
	GlzAmp->InputSampleStereo(inpSplA, inpSplB);

	GlzAmp->RunPlugin();

	outputDoubleL = (fsx) GlzAmp->GetStereoOutLeft();
	outputDoubleR = (fsx) GlzAmp->GetStereoOutRight();
	outputFloatL = flx (outputDoubleL);
	outputFloatR = flx (outputDoubleR);
}
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::CheckParams()
{
	...
	if(isOnlineOld != isOnline)
	{
		...
		if(isOnline == true)
		{
			if(overSmpRate[1] == 1) GlzAmp->SetIntParam(ScPrmI_OverSampling_Rate, SCv_OverSampling_1x);
			if(overSmpRate[1] == 2) GlzAmp->SetIntParam(ScPrmI_OverSampling_Rate, SCv_OverSampling_2x);
			if(overSmpRate[1] == 4) GlzAmp->SetIntParam(ScPrmI_OverSampling_Rate, SCv_OverSampling_4x);
			if(overSmpRate[1] == 8) GlzAmp->SetIntParam(ScPrmI_OverSampling_Rate, SCv_OverSampling_8x);
			if(overSmpRate[1] == 16) GlzAmp->SetIntParam(ScPrmI_OverSampling_Rate, SCv_OverSampling_16x);
			if(overSmpRate[1] == 0) GlzAmp->SetIntParam(ScPrmI_OverSampling_Rate, SCv_OverSampling_OFF);
			if(bDepth[1] == 32) GlzAmp->SetIntParam(ScPrmI_FP_Precision_Bit_Depth, SCv_Bit_Depth_32);
			if(bDepth[1] == 64) GlzAmp->SetIntParam(ScPrmI_FP_Precision_Bit_Depth, SCv_Bit_Depth_64);
			if(bDepth[1] == 128) GlzAmp->SetIntParam(ScPrmI_FP_Precision_Bit_Depth, SCv_Bit_Depth_128);
		} else
		{
			if(overSmpRate[0] == 1) GlzAmp->SetIntParam(ScPrmI_OverSampling_Rate, SCv_OverSampling_1x);
			if(overSmpRate[0] == 2) GlzAmp->SetIntParam(ScPrmI_OverSampling_Rate, SCv_OverSampling_2x);
			if(overSmpRate[0] == 4) GlzAmp->SetIntParam(ScPrmI_OverSampling_Rate, SCv_OverSampling_4x);
			if(overSmpRate[0] == 8) GlzAmp->SetIntParam(ScPrmI_OverSampling_Rate, SCv_OverSampling_8x);
			if(overSmpRate[0] == 16) GlzAmp->SetIntParam(ScPrmI_OverSampling_Rate, SCv_OverSampling_16x);
			if(overSmpRate[0] == 0) GlzAmp->SetIntParam(ScPrmI_OverSampling_Rate, SCv_OverSampling_OFF);
			if(bDepth[0] == 32) GlzAmp->SetIntParam(ScPrmI_FP_Precision_Bit_Depth, SCv_Bit_Depth_32);
			if(bDepth[0] == 64) GlzAmp->SetIntParam(ScPrmI_FP_Precision_Bit_Depth, SCv_Bit_Depth_64);
			if(bDepth[0] == 128) GlzAmp->SetIntParam(ScPrmI_FP_Precision_Bit_Depth, SCv_Bit_Depth_128);
		}
	}
	...
	for(int x = 0; x < kMaxNumParameters; x++)
	{
		if(oldParams[x] != params[x])
		{
			...
			if(x == kMasterVol) GlzAmp->SetFpParam(ScPrmF_Master_Volume, params[x]);
			if(x == kInputVol) GlzAmp->SetFpParam(ScPrmF_Input_Vol, params[x]);
			if(x == kGainControl) GlzAmp->SetFpParam(ScPrmF_Drive_Gain, params[x]);
			if(x == kEQstackLow) GlzAmp->SetFpParam(ScPrmF_EQ_Low, params[x]);
			if(x == kEQstackMid) GlzAmp->SetFpParam(ScPrmF_EQ_Mid, params[x]);
			if(x == kEQstackHigh) GlzAmp->SetFpParam(ScPrmF_EQ_High, params[x]);
			if(x == kEQstackContour) GlzAmp->SetFpParam(ScPrmF_EQ_Contour, params[x]);
			if(x == kEQstackPres) GlzAmp->SetFpParam(ScPrmF_EQ_Presence, params[x]);
			if(x == kPreEQstackLow) GlzAmp->SetFpParam(ScPrmF_PreEQ_Low, params[x]);
			if(x == kPreEQstackMid) GlzAmp->SetFpParam(ScPrmF_PreEQ_Mid, params[x]);
			if(x == kPreEQstackHigh) GlzAmp->SetFpParam(ScPrmF_PreEQ_High, params[x]);
			if(x == kPreEQstackContour) GlzAmp->SetFpParam(ScPrmF_PreEQ_Contour, params[x]);
			if(x == kPreEQstackPres) GlzAmp->SetFpParam(ScPrmF_PreEQ_Presence, params[x]);
			...
			if(x == kBtnBright)
			{
				if(params[x] == 1.f) GlzAmp->SetIntParam(ScPrmI_Bright, 1);
				if(params[x] == 0.f) GlzAmp->SetIntParam(ScPrmI_Bright, 0);
			}
			if(x == kInternalCabOff)
			{
				if(params[x] == 1.f) GlzAmp->SetIntParam(ScPrmI_CabMic_Active, 1);
				if(params[x] == 0.f) GlzAmp->SetIntParam(ScPrmI_CabMic_Active, 0);
			}
			if((x >= kInternalCabA) && (x <= kInternalCabF))
			{
				if(params[x] == 1.f)
				{
					GlzAmp->SetIntParam(ScPrmI_CabMic_Num, x-kInternalCabA);
					CycleCabsOff(x);
				} else
				{
			...
			{
				if(params[x] == 1.f) GlzAmp->SetIntParam(ScPrmI_TStack_Shape, 1);
				if(params[x] == 0.f) GlzAmp->SetIntParam(ScPrmI_TStack_Shape, 0);
			}
			if(x == kDynamicEnabled)
			{
				if(params[x] == 1.f) GlzAmp->SetIntParam(ScPrmI_TStack_Dynamic, 1);
				if(params[x] == 0.f) GlzAmp->SetIntParam(ScPrmI_TStack_Dynamic, 0);
			}
			if(x == kToneStShapedA)
			{
				if(params[x] == 1.f)
			...
			if(x == kToneStShapedB)
			{
				if(params[x] == 1.f)
				{
					GlzAmp->SetIntParam(ScPrmI_TStack_ShapeNum, 1);
			...
			if(x == kToneStShapedC)
			{
				if(params[x] == 1.f)
				{
					GlzAmp->SetIntParam(ScPrmI_TStack_ShapeNum, 2);
					setParameterAutomated(kToneStShapedB, 0.f);
					setParameterAutomated(kToneStShapedA, 0.f);
					setParameterAutomated(kToneStShapedD, 0.f);
					setParameterAutomated(kToneStShapedE, 0.f);
			...
			if(x == kToneStShapedD)
			{
				if(params[x] == 1.f)
				{
					GlzAmp->SetIntParam(ScPrmI_TStack_ShapeNum, 3);
					setParameterAutomated(kToneStShapedB, 0.f);
					setParameterAutomated(kToneStShapedC, 0.f);
					setParameterAutomated(kToneStShapedA, 0.f);
					setParameterAutomated(kToneStShapedE, 0.f);
			...
			if(x == kToneStShapedE)
			{
				if(params[x] == 1.f)
				{
					GlzAmp->SetIntParam(ScPrmI_TStack_ShapeNum, 4);
					setParameterAutomated(kToneStShapedB, 0.f);
					setParameterAutomated(kToneStShapedC, 0.f);
					setParameterAutomated(kToneStShapedD, 0.f);
					setParameterAutomated(kToneStShapedA, 0.f);
			...
			if(x == kToneStShapedF)
			{
				if(params[x] == 1.f)
				{
					GlzAmp->SetIntParam(ScPrmI_TStack_ShapeNum, 5);
					setParameterAutomated(kToneStShapedB, 0.f);
					setParameterAutomated(kToneStShapedC, 0.f);
					setParameterAutomated(kToneStShapedD, 0.f);
					setParameterAutomated(kToneStShapedE, 0.f);
			...
			if(x == kSmpBitOn32)
			{
				if(params[x] == 1.f)
				{
					...
					updateDisplay();
					bDepth[1] = 32;
					if(isOnline) GlzAmp->SetIntParam(ScPrmI_FP_Precision_Bit_Depth, SCv_Bit_Depth_32);
				} else
				{
			...
			if(x == kSmpBitOn64)
			{
				if(params[x] == 1.f)
				{
					...
					updateDisplay();
					bDepth[1] = 64;
					if(isOnline) GlzAmp->SetIntParam(ScPrmI_FP_Precision_Bit_Depth, SCv_Bit_Depth_64);
				} else
				{
			...
			if(x == kSmpBitOn128)
			{
				if(params[x] == 1.f)
				{
					...
					updateDisplay();
					bDepth[1] = 128;
					if(isOnline) GlzAmp->SetIntParam(ScPrmI_FP_Precision_Bit_Depth, SCv_Bit_Depth_128);
				} else
				{
			...
			if(x == kTStackBeforePower)
			{
				if(params[x] == 1.f)
				{
					GlzAmp->SetIntParam(ScPrmI_TStack_Before_Power, 1);
			...
			if(x == kTStackAfterPower)
			{
				if(params[x] == 1.f)
				{
					GlzAmp->SetIntParam(ScPrmI_TStack_Before_Power, 0);
	...
	if(isRunningStereoOld != isRunningStereo)
	{
		...
		if(isRunningStereo == true) GlzAmp->SetIntParam(ScPrmI_Stereo, 1);
		else GlzAmp->SetIntParam(ScPrmI_Stereo, 0);
	}
}
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::CycleCabsOff(int prm)
{
...
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::SetOnlineSampling(const int inp)
{
...
//! -------------------------------------------------------------------------------------------------------
void ScGLZ60::SetOfflineSampling(const int inp)
{
