...
ToneStack_J::ToneStack_J(const fsx nRate, const SC_ToneStack_Params inpParams) :
...
void ToneStack_J::setNewSampleRate(const fhx nRate)
{
	ToneStRate = fhx (nRate);
...
void ToneStack_J::setBandValue(const int bNum, fsx value)
{
	if(bNum < 0) return;
	if(bNum > 5) return;
...
void ToneStack_J::setEvenShaped(const bool isShaped)
{
	if(setShaped != isShaped) ClearBuffs();
	setShaped = isShaped;
}
void ToneStack_J::ClearBuffs()
{
	h_DynEnvPos = fhx (0.0);
	s_DynEnvPos = fsx (0.0);
	l_DynEnvPos = flx (0.f);
...
void ToneStack_J::SetPrecision(int nLevel)
{
...
void ToneStack_J::RunEQ_Controls()
{
	if(fpPrec == 0)
	{
...
void ToneStack_J::l_SetDynamicQ(const flx inp)
{
...
void ToneStack_J::s_SetDynamicQ(const fsx inp)
{
...
void ToneStack_J::h_SetDynamicQ(const fhx inp)
{
