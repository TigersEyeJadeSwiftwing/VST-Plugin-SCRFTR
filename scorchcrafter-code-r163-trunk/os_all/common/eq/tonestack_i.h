#ifndef __SC_eq_tstack_i__
#define __SC_eq_tstack_i__
...
#define SC_NUM_TSTACK_I_IMPULSES_AMP 6
#define SC_NUM_TSTACK_I_IMPULSES_CAB 6
#define SC_NUM_TSTACK_I_IMPULSES 6

class ToneStack_I
{
public:
	ToneStack_I(fsx nRate, const SC_ToneStack_Params inpParams);
	~ToneStack_I();
	...
	void SetAmpShapeNum(int inp) { curAmp=inp; if(inp>SC_NUM_TSTACK_I_IMPULSES_AMP) inp=SC_NUM_TSTACK_I_IMPULSES_AMP-1; };
	void SetCabMicNum(int inp) { curCab=inp; if(inp>SC_NUM_TSTACK_I_IMPULSES_CAB) inp=SC_NUM_TSTACK_I_IMPULSES_CAB-1; };
...
private:
	...
	Cst_EQ *AmpHeadEQ[SC_NUM_TSTACK_I_IMPULSES_AMP][SCr_TStack_I_Max_Head_Elements];
	flx l_ampHead_smp[SCr_TStack_I_Max_Head_Elements];
	fsx s_ampHead_smp[SCr_TStack_I_Max_Head_Elements];
	fhx h_ampHead_smp[SCr_TStack_I_Max_Head_Elements];
	...
	int impLength[SC_NUM_TSTACK_I_IMPULSES];

	int smpPosC;

	flx volAdjC_l[SC_NUM_TSTACK_I_IMPULSES];
	fsx volAdjC_s[SC_NUM_TSTACK_I_IMPULSES];
	fhx volAdjC_h[SC_NUM_TSTACK_I_IMPULSES];
};
