#include "tube_bank_g.h"

using namespace ScMath;

Tube_Bank_G::Tube_Bank_G(const SC_TubeDistortion_Params *initParams, fsx spRate) : SampleBank_Cst(spRate),
	params(*initParams),
...
void Tube_Bank_G::InitTubes()
{
	clearBuffs();
}
void Tube_Bank_G::setNSampleRate(fsx nSRate)
{
...
void Tube_Bank_G::SetGain(const fsx nGain)
{
	h_Gain = fhx (nGain);
...
void Tube_Bank_G::zeroAllSamples()
{
	SampleBank_Cst::zeroAllSamples();
}
void Tube_Bank_G::setOverSampling(const int oSLevel)
{
...
void Tube_Bank_G::RunPreAmp()
{
	return;
}
void Tube_Bank_G::RunPowerAmp()
{
	return;
}
void Tube_Bank_G::ApplyDistortion()
{
	if(osRate <= 0) return;
	if(osRate >= 6) return;
//! ----------------------------------------------------------------------------------------------------------------
...
//!		Pre-Amp
			...
			xPre = (flx) l_Diode_U(xPre, l_Pre_Grit);
			xPre = flx (1.0f - xPre);
...
//!		Pre-Amp
			...
			xPre = (fsx) s_Diode_U(xPre, s_Pre_Grit);
			xPre = fsx (1.0 - xPre);
...
//!		Pre-Amp
			...
			if (h_smp >= 0.0)
			{
				PreFiltA->h_InputSmpl(fhx (0.0000010));
			} else
			{
				PreFiltA->h_InputSmpl(fhx (1.0));
				PreFiltA->h_Run();
				PreFiltA->h_Run();
				PreFiltA->h_Run();
			}
			PreFiltA->h_Run();
			fhx xPre = fhx (1.0 - PreFiltA->h_GetOutput());
			...
			h_smp = fhx ( h_smp * (dAp + h_abs(h_smp)) / ( (dAp * (1.0 + PreFiltB->h_GetOutput())) + (dAp * h_abs(h_smp)) + (h_smp*h_smp) ) );
...
//!		Power-Amp
			...
			if (h_smp >= 0.0)
			{
				PowerFiltA->h_InputSmpl(fhx (0.0000010));
			} else
			{
...
void Tube_Bank_G::SetAmpVt(const fsx inp)
{
	return;
}
