#ifndef __dist_tube_cmp_diode_S__
#define __dist_tube_cmp_diode_S__
...
public:
	...
	SC_Diode_S(const fsx hard, const fsx slope, const fsx freq = 90.0, const fsx spRate = 44100.0, const fsx clip = 0.80) :
	h_hard(fhx (hard)),
	h_slope(fhx (slope)),
	s_hard(fsx (hard)),
	s_slope(fsx (slope)),
	...
	void l_Run(const flx feedBack = 0.0f)
	{
		...
		l_in += flx (l_in * (l_spValA - l_spValB));
	...
	void s_Run(const fsx feedBack = 0.0)
	{
		...
		s_in += fsx (s_in * (s_spValA - s_spValB));
	...
	void h_Run(const fhx feedBack = 0.0)
	{
		...
		h_in += fhx (h_in * (h_spValA - h_spValB));
