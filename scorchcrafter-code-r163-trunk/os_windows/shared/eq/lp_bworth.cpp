...
void BTWorthLP::setSampleRate(const fsx inpSRate)
{
	...
	wp = fhx (fhx (sT[2]) * (fhx) tanl(fhx (sT[3]) * fhx (cutOff * 0.50)));
...
//! -------------------------------------------------------------------------------------------------------
BTWorthLP_SD::BTWorthLP_SD(fsx inpFreq, float inpSRate) :
	PI((fsx) ((fsx) 4.00 * (fsx) atanl((fsx) 1.00))),
	hPI((fsx) ((fsx) PI * (fsx) 0.50)),
	PId(fsx (fsx (PI) * fsx (2.0))),
	zeroF(fsx (0.00000000000001)),				//! Used to avoid denormal-related issues in parts of the code
	cutOff(fsx (inpFreq))
{
	if(float (inpSRate) < float (0.0f)) setSampleRate(float (44100.0f));
	else setSampleRate(float (inpSRate));
}
//! -------------------------------------------------------------------------------------------------------
void BTWorthLP_SD::setSampleRate(const float inpSRate)
{
	sRate = fsx (inpSRate * 0.50);
	if(fsx (sRate) < fsx (22050.0)) sRate = fsx (22050.0);
	...
	wp = fsx (fsx (sT[2]) * (fsx) tan(fsx (sT[3]) * fsx (cutOff * 0.50)));
...
//! -------------------------------------------------------------------------------------------------------
fsx BTWorthLP_SD::runFilter(fsx inp)
{
	if(fsx (inp) == fsx (0.0)) inp = fsx (zeroF);
...
//! -------------------------------------------------------------------------------------------------------
BTWorthLP_HD::BTWorthLP_HD(fhx inpFreq, float inpSRate) :
	PI((fhx) ((fhx) 4.00 * (fhx) atanl((fhx) 1.00))),
	hPI((fhx) ((fhx) PI * (fhx) 0.50)),
	PId(fhx (fhx (PI) * fhx (2.0))),
...
//! -------------------------------------------------------------------------------------------------------
void BTWorthLP_HD::setSampleRate(const float inpSRate)
{
	sRate = fhx (inpSRate * 0.50);
	if(fhx (sRate) < fhx (22050.0)) sRate = fhx (22050.0);
	if(fhx (sRate) > fhx (1536000.0)) sRate = fhx (1536000.0);
...
//! -------------------------------------------------------------------------------------------------------
fhx BTWorthLP_HD::runFilter(fhx inp)
{
	if(fhx (inp) == fhx (0.0)) inp = fhx (zeroF);
...
//! -------------------------------------------------------------------------------------------------------
void BTWorthLP_HD::changeFreq(fhx nFreq)
{
	cutOff = fhx (nFreq);
