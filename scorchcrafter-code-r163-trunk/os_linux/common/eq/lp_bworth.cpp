...
BTWorthLP_LD::BTWorthLP_LD(fsx inpFreq, fsx inpSRate) :
	...
	zeroF(flx (0.000000000000000001f)),				//! Used to avoid denormal-related issues in parts of the code
	cutOff(flx (inpFreq))
{
...
void BTWorthLP_LD::setSampleRate(const fsx inpSRate)
{
	...
	wp = flx (flx (sT[2]) * (flx) tan(flx (sT[3]) * flx (cutOff * 0.50f)));
...
{
	if(flx (inp) == fsx (0.0f)) inp = fsx (zeroF);
...
void BTWorthLP_LD::changeFreq(fsx nFreq)
{
	...
	sT[2] = flx (flx (2.0f) * flx (sRate));
	sT[3] = flx (flx (PI) / flx (sRate));
	wp = flx (flx (sT[2]) * (flx) tan(flx (sT[3]) * flx (cutOff * 0.50)));
...
BTWorthLP_SD::BTWorthLP_SD(fsx inpFreq, fsx inpSRate) :
	...
	zeroF(fsx (0.00000000000000000001)),				//! Used to avoid denormal-related issues in parts of the code
	cutOff(fsx (inpFreq))
{
	if(fsx (inpSRate) < fsx (0.0)) setSampleRate(fsx (44100.0));
	else setSampleRate(fsx (inpSRate));
...
BTWorthLP_HD::BTWorthLP_HD(fsx inpFreq, fsx inpSRate) :
	...
	hPI((fhx) ((fhx) PI * (fhx) 0.50)),
	PId(fhx (fhx (PI) * fhx (2.0))),
	zeroF(fhx (0.000000000000000000000000000001)),				//! Used to avoid denormal-related issues in parts of the code
	cutOff(fhx (inpFreq))
{
	if(fsx (inpSRate) < fsx (0.0)) setSampleRate(fsx (44100.0));
	else setSampleRate(fsx (inpSRate));
